-- Workflow Form Configuration Schema
-- This script adds the form-workflow configuration system
-- Run this after workflow-schema.sql
...
-- Add index for better workflow assignment queries
IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'IX_FormDefinitions_WorkflowDefinitionId' AND object_id = OBJECT_ID('FormDefinitions'))
BEGIN
    CREATE INDEX IX_FormDefinitions_WorkflowDefinitionId ON FormDefinitions(WorkflowDefinitionId);
    PRINT 'Added index for WorkflowDefinitionId on FormDefinitions.';
END
GO

PRINT 'Workflow Form Configuration schema applied successfully.';
